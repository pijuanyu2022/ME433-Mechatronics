...
void set_Pin(unsigned char register_value, unsigned char value){
    i2c_master_start();
    i2c_master_send(0b11010100);  // send address for writing
...
unsigned char read_Pin(unsigned char register_value){
    ...
    i2c_master_send(register_value);    // send the register
    i2c_master_restart();
    i2c_master_send(0b11010101);  // send address for reading
    unsigned char A = i2c_master_recv();  // received message from chip
    i2c_master_ack(1);            // acknowledge with a 1
    i2c_master_stop();    
    return A;
}
void read_Multiple(unsigned char reg, signed short * data, int length){
    i2c_master_start();
    i2c_master_send(0b11010100); // send a byte (either an address or data)
    i2c_master_send(reg);
    i2c_master_restart();
    ...
    A[length] = i2c_master_recv();
    i2c_master_ack(1); //acknowledge with a 1
    i2c_master_stop();
   
    int j = 0;
    for (j=0; j<7; j++){
        data[j] = (A[(2*j)+1] << 8) | A[2*j];
    }
    }
