...
// drawChar function
void draw_Char(unsigned short x, unsigned short y, unsigned short color, unsigned char letter){
    int i=0, j=0;
    for(i=0; i<5 ;i++){
        for(j=0; j<8; j++){
            if(((ASCII[letter - 0x20][i] >> j) & 0b1) == 1){
...
void draw_String(unsigned short x, unsigned short y, unsigned short color, char *m){
    int i=0;
    for (i=0; i<strlen(m); i++){
        draw_Char(x+(i*5), y, color, m[i]);
    }
}

void draw_Bar(unsigned short x, unsigned short y, int i, int length, int width){
    int k=0, j=0;
    for(k=0; k<length; k++){
        for(j=0; j<width; j++){
            if(k<i){
                LCD_drawPixel(x+k, y+j, RED);
            }
            else {
                LCD_drawPixel(x+i, y+j, BLUE);
                LCD_drawPixel(x+k, y+j, WHITE);
            }
        }
    }
}